...
/* -------- Test: DAC device is ready -------- */
static void dac_device_ready_test(void)
{
	const struct device *dev = DEVICE_DT_GET(DAC_NODE);

	LOG_INF("=== DAC Device Ready Test ===");

	zassert_not_null(dev, "DAC device handle is NULL");
	zassert_true(device_is_ready(dev), "DAC device is not ready");

	LOG_INF("DAC device ready test PASSED");
}

/* -------- Test: DAC channel setup with valid 12-bit resolution -------- */
static void dac_channel_setup_test(void)
{
	int ret;
	const struct device *dev = DEVICE_DT_GET(DAC_NODE);

	LOG_INF("=== DAC Channel Setup Test ===");
	...
	LOG_INF("DAC channel setup test PASSED");
}
...
static void dac_dual_channel_independent_write_test(void)
{
	...
	zassert_true(device_is_ready(dac0), "DAC0 not ready");
	zassert_true(device_is_ready(dac1), "DAC1 not ready");

	ret = dac_channel_setup(dac0, &cfg);
	zassert_equal(ret, 0, "DAC0 setup failed [%d]", ret);

	ret = dac_channel_setup(dac1, &cfg);
	zassert_equal(ret, 0, "DAC1 setup failed [%d]", ret);
	...
	for (int i = 0; i < ARRAY_SIZE(dac0_values); i++) {
		ret = dac_write_value(dac0, DAC_CHANNEL_ID, dac0_values[i]);
		zassert_equal(ret, 0, "DAC0 write %d failed [%d]",
			      dac0_values[i], ret);

		ret = dac_write_value(dac1, DAC_CHANNEL_ID, dac1_values[i]);
...
static void dac_dual_channel_simultaneous_test(void)
{
	...
	for (uint32_t val = 0; val <= DAC_MAX_INPUT; val += DAC_RAMP_STEP) {
		...
		ret = dac_write_value(dac0, DAC_CHANNEL_ID, val);
		zassert_equal(ret, 0, "DAC0 write %u failed [%d]", val, ret);

		ret = dac_write_value(dac1, DAC_CHANNEL_ID, val_inv);
...
static void *test_dac_basic_setup(void)
{
	...
	dac_dev = DEVICE_DT_GET(DAC_NODE);
	zassert_not_null(dac_dev, "DAC device handle is NULL");
	zassert_true(device_is_ready(dac_dev), "DAC device not ready");
	return NULL;
}
...
/* R-32_1: DAC device ready */
ZTEST(test_dac_basic, test_dac_device_ready)
{
	dac_device_ready_test();
}

/* R-32_1: Channel setup with valid resolution */
ZTEST(test_dac_basic, test_dac_channel_setup)
{
	dac_channel_setup_test();
}
