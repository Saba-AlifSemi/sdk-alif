...
static void write_without_erase_test(void)
{
	...
	/* Verify - data may be corrupted due to not erasing first */
	e_count = verify_data(w_buf, r_buf, len, "write_without_erase");

	/* Note: This test documents behavior when writing without erase.
	 * Flash bits can only be flipped from 1 to 0, not 0 to 1 without erase.
	 * The result depends on previous flash state.
	 */
	if (e_count == 0) {
		LOG_INF("Write without erase OK (already erased)");
	} else {
		LOG_INF("Write without erase: %d mismatches",
			e_count);
		LOG_INF("Expected for flash without prior erase");
	}
