...
{
	int ret, e_count;
	uint8_t min_buf[16]; /* Covers any write_block_size value */
	...
	/* Test minimum write size (write_block_size) */
	memset(min_buf, 0xAA, sizeof(min_buf));
	ret = flash_write(flash_dev, SPI_FLASH_TEST_REGION_OFFSET,
			  min_buf, sizeof(min_buf));
	zassert_equal(ret, 0, "Minimum size write failed [%d]", ret);

	memset(r_buf, 0, sizeof(min_buf));
	ret = flash_read(flash_dev, SPI_FLASH_TEST_REGION_OFFSET,
			 r_buf, sizeof(min_buf));
	zassert_equal(ret, 0, "Minimum size read failed [%d]", ret);

	e_count = verify_data(min_buf, r_buf, sizeof(min_buf),
			      "min_size");
	zassert_equal(e_count, 0,
		     "Min size verify failed: %d mismatches",
		     e_count);

	LOG_INF("Minimum size operation PASSED");

	/* Test page size write - erase target sector first */
	ret = flash_erase(flash_dev, SPI_FLASH_TEST_REGION_OFFSET + 0x1000,
			  SPI_FLASH_SECTOR_SIZE);
	zassert_equal(ret, 0, "Page sector erase failed [%d]", ret);

	memset(page_buf, 0x55, sizeof(page_buf));
	ret = flash_write(flash_dev, SPI_FLASH_TEST_REGION_OFFSET + 0x1000,
			  page_buf, sizeof(page_buf));
	zassert_equal(ret, 0, "Page size write failed [%d]", ret);

	memset(r_buf, 0, sizeof(page_buf));
	ret = flash_read(flash_dev, SPI_FLASH_TEST_REGION_OFFSET + 0x1000,
			r_buf, sizeof(page_buf));
	zassert_equal(ret, 0, "Page size read failed [%d]", ret);

	e_count = verify_data(page_buf, r_buf, sizeof(page_buf),
			      "page_size");
	zassert_equal(e_count, 0,
		     "Page size verify failed: %d mismatches",
		     e_count);

	LOG_INF("Page size operation PASSED");
}
