...
static void multi_page_test(const struct device *dev)
{
	...
	ret = flash_erase(dev, SPI_FLASH_TEST_REGION_OFFSET,
			  SPI_FLASH_SECTOR_SIZE);
	...
	LOG_INF("Test 3: Flash write");
	LOG_INF("Attempting to write %zu bytes", len);

	start_time = k_uptime_get();
	ret = flash_write(dev, SPI_FLASH_TEST_REGION_OFFSET, w_buf, len);
	end_time = k_uptime_get();
	zassert_equal(ret, 0, "Error: Flash write Failed [%d]", ret);

	tm_taken_ms = end_time - start_time;
	LOG_INF("Write operation test 3: completed in %lld ms",
		tm_taken_ms);

	LOG_INF("Test 3: Flash read");

	memset(r_buf, 0, len);
	ret = flash_read(dev, SPI_FLASH_TEST_REGION_OFFSET, r_buf, len);
	zassert_equal(ret, 0, "Error: Flash read Failed [%d]", ret);
...
static void data_pattern_test(void)
{
	...
	ret = flash_write(flash_dev,
			  SPI_FLASH_TEST_REGION_OFFSET,
			  w_buf, BUFF_SIZE);
	zassert_equal(ret, 0, "All-zeros write failed [%d]", ret);
	...
	ret = flash_write(flash_dev,
			  SPI_FLASH_TEST_REGION_OFFSET,
			  w_buf, BUFF_SIZE);
	zassert_equal(ret, 0, "All-ones write failed [%d]", ret);
	...
	ret = flash_write(flash_dev,
			  SPI_FLASH_TEST_REGION_OFFSET,
			  w_buf, BUFF_SIZE);
	zassert_equal(ret, 0, "0xAA pattern write failed [%d]", ret);
	...
	ret = flash_write(flash_dev,
			  SPI_FLASH_TEST_REGION_OFFSET,
			  w_buf, BUFF_SIZE);
	zassert_equal(ret, 0, "0x55 pattern write failed [%d]", ret);
